...
class BleService extends ChangeNotifier {
  ...
  bool _isConnecting = false; // FIX 1: guard against overlapping connect() calls
  ...
  Future<void> startScan() async {
    if (_isScanning || _isConnected || _isConnecting) return; // FIX 1: also check _isConnecting
      ...
      _scanSubscription = FlutterBluePlus.scanResults.listen((results) {
        for (ScanResult result in results) {
          if (result.device.platformName == targetDeviceName) {
            ...
            _scanSubscription?.cancel(); // FIX 2: cancel listener to prevent duplicate connect calls
  ...
  Future<void> _connectToDevice(BluetoothDevice device) async {
    // FIX 1: prevent concurrent connection attempts — this is the main cause of
    // fbp-code:10 (connection canceled). Without this guard, the scan listener,
    // bonded device path, and adapter state listener can all fire _connectToDevice
    // at the same time. The second connect() cancels the first, triggering the error.
    ...
    try {
      ...
      // FIX 3: try to disconnect after a failed connect() to free the GATT client
      // slot. Without this, each failed attempt leaks a slot, and after ~30 leaks
      // you get fbp-code:257. This won't fix already-leaked slots, but prevents
      // new leaks going forward.
      try {
        await device.disconnect();
      } catch (_) {}
